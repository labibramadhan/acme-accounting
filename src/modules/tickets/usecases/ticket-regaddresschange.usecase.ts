  ...
  implements ITicketRegAddressChangeUsecase
{
  ...
  async create(ticket: CreateTicketDTO): Promise<TicketDTO> {
    ...
    const userRole: RoleEnum = RoleEnum.corporateSecretary;
    const category: TicketCategory = TicketCategory.corporate;

    let assignees = await this.userRepository.findAll({
    ...
    if (!assignees.length) {
      assignees = await this.userRepository.findAll({
        where: { companyId, role: RoleEnum.director },
        order: [['createdAt', 'DESC']],
        limit: 2,
      });
    }
    ...
    if (!assignees.length)
      throw new ConflictException(
        `Cannot find user with role ${userRole} or ${RoleEnum.director} to create a ticket`,
      );
