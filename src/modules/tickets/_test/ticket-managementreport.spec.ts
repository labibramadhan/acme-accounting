...
describe('Tickets - Type: Management Report', () => {
  ...
  describe('create', () => {
    it('create ticket', async () => {
      ...
      ticket.type = TicketType.managementReport;
      ...
      expect(result.category).toEqual(TicketCategory.accounting);
    ...
    it('throw if there is no user to assign', () => {
      ...
      expect(() => controller.create(ticket)).rejects.toThrow(
        `Cannot find user with role ${RoleEnum.accountant} to create a ticket`,
      );
    });
    it('if there are multiple users, assign the last one', async () => {
      const lastUser = users[0];
      const spyTicketCreate = jest.spyOn(TicketRepository.prototype, 'create');
      const spyUserFindAll = jest.spyOn(UserRepository.prototype, 'findAll');
      spyUserFindAll.mockResolvedValue(users);
      ...
      expect(spyTicketCreate).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          companyId: ticket.companyId,
          category: TicketCategory.accounting,
          assigneeId: lastUser.id,
          status: TicketStatus.open,
        }),
      );

      expect(result.id).toBeDefined();
      expect(result.assigneeId).toEqual(lastUser.id);
      expect(result.companyId).toEqual(company.id);
      expect(result.category).toEqual(TicketCategory.accounting);
      expect(result.status).toEqual(TicketStatus.open);
    });
